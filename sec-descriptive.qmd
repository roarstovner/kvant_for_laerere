...
Videre kan man dele inn deskriptiv statistikk i to

- man kan regne ut tall for å beskrive dataene, for eksempel gjennomsnittet av en variabel
- man kan lage et diagram som viser dataene, for eksempel et søylediagram. Diagrammer kalles også figurer og plott.

I de neste to kapitlene lærer du om vanlige tall og diagrammer man bruker for å beskrive data.
...
Nå, ble du noe klokere? Jeg tipper du ikke ble noe klokere, og at du lengter etter å få dataene omarbeidet og presentert på en eller annen måte. Det er dette som er deskriptiv statistikk, det å gjøre rå data om til tall og figurer som gir innsikt.

Nå til dags gjør man all statistikk på en datamaskin, så la oss vise dataene i et statistikkprogram, jamovi. Etter å ha åpnet filen *[ICCS.omv](data_and_tables/ICCS.omv) ser det ut som i *@fig-descriptive-ICCS-data.
...
Det tallet elevene får på variabelene _Likestilling_ gir altså bare mening i lys av disse påstandene. Hadde vi påstandene vært annerledes, hadde tallet vært et annet og kanskje fortalt en annen historie. I tillegg regner ICCS om elevenes svar på disse påstandene til en samlevariabel. Den går fra ca. 15 (svært imot likestilling mellom kjønnene) til ca. 67 (svært for). Hvis du er interessert i å vite hvordan ICCS gjør denne omregningen er det bare å slå opp i dokumentasjonen, men vi trenger det ikke for denne boka.

Nå skal vi gjøre deskriptiv statistikk på denne variabelen, og vi starter med histogrammer.

### Histogrammer {#sec-histogram}

**Histogrammer** er blant de mest grunnleggende og nyttige måtene å visualisere data på. De fungerer for tallvariable, altså variable på intervall- eller forholdstallsnivå, og gir et godt inntrykk av hvordan tallene fordeler seg. De fleste kjenner nok til histogrammer, siden de brukes så ofte, men la meg likevel forklare hvordan de fungerer.

Prinsippet er enkelt: Du deler opp de mulige verdiene i intervaller og teller hvor mange observasjoner som faller innenfor hvert intervall. Denne tellingen kalles frekvensen eller tettheten til intervallet og vises som en vertikal stolpe. Jeg tror noe av grunnen til at de oppfattes som så enkle er at de minner om stolpediagrammer. @fig-diagram-ICCS-histogram viser et histogram av _Likestilling_. Vi ser at de fleste elevene skårer høyt på påstanden, men at en betydelig andel svarer mye lavere. Stolpen helt til høyre representerer for eksempel rundt 6000 elever som fikk max poengsum; det kan du lese av på $y$-aksen.
...
-   **For nominelle variable** kan du hverken bruke gjennomsnitt eller median, typetall er det eneste sentralmålet som fungerer.

-   **For ordinale variable** vil medianen stort sett være det beste valget. Medianen bruker kun rekkefølgeinformasjonen i dataene dine (hvilke tall som er større) og trenger ikke eksakte tallverdier. Dette passer perfekt for ordinale data. Gjennomsnittet derimot, er avhengig av de presise tallverdiene. Det er likevel ganske vanlig å regne gjennomsnitt av variable på ordinalnivå. Da setter man, for eksempel, det laveste nivået til tallet 1, det neste nivået til tallet 2, osv.
...
Dette illustreres også i @fig-descriptive-weight, hvor du kan se at når histogrammet er asymmetrisk, i dette tilfellet med hovedtyngden til venstre og noen ekstreme verdier langt til høyre, ligger medianen nærmere hovedtyngten av dataene, mens gjennomsnittet blir trukket mot "halen" der de ekstreme verdiene befinner seg.
...
Hvis du kjenner begrepet **persentil** fra før, kan du tenke på det slik: den 25. persentilen til en variabel er det minste tallet som har 25% av verdiene liggende under seg. En **kvartil** er bare navnet på 25., 50. og 75. persentil, og disse kalles henholdsvis første, andre og tredje kvartil.
...
Dette er en effektiv måte å vise sentraltendens og spredning. Medianen viser hva midtpunktet i dataene er og kvartilbredden viser hvor spredt halvparten av verdiene ligger rundt medianen.

Kvartilbredden har den samme gode egenskapen til medianen, den blir ikke påvirket av ekstreme verdier. Bare tenk på kvartilbredden når Haaland satte seg ved bordet i eksempelet i @sec-descriptive-mean-or-median. Merk at kvartilbredden ikke fungerer like godt for å vise spredningen rundt et gjennomsnitt. Årsaken er at gjennomsnittet kan ligge utenfor 25. og 75. persentil, akkurat slik den vil gjøre i Haaland-eksempelet.
...
avvik <- likestilling - lik_mean
ss <- sum(avvik^2)
sd_eks_var <- ss / lik_n
sd_eks <- sqrt(sd_eks_var)
avvik_str <- paste(format(avvik, trim = TRUE), collapse = "; ")
```

La oss regne det ut for hånd på de samme `r lik_n` verdiene av _Likestilling_ som vi brukte for gjennomsnittet. Gjennomsnittet av dem var `r lik_mean`. **Avviket** til en observasjon er hvor langt den ligger fra gjennomsnittet, altså verdien minus gjennomsnittet. For de `r lik_n` verdiene blir avvikene:

> `r avvik_str`.

Vi vil sammenfatte avvikene til ett tall som forteller hvor stort et typisk avvik er. Det nærliggende er å ta gjennomsnittet av dem, men det går ikke: de positive og negative avvikene opphever hverandre, og summen blir alltid nøyaktig null. Trikset er å **kvadrere** hvert avvik først, slik at alt blir positivt. Det første avviket, `r format(avvik[1], trim = TRUE)`, blir for eksempel `r format(round(avvik[1]^2, 1), nsmall = 1)` etter kvadrering. Så legger vi sammen alle de kvadrerte avvikene, deler på antallet `r lik_n`, og tar til slutt kvadratroten for å komme tilbake til samme størrelsesorden som dataene:

$$ s = \sqrt{\frac{`r format(round(ss, 2), nsmall = 2)`}{`r lik_n`}} = \sqrt{`r format(round(sd_eks_var, 2), nsmall = 2)`} = `r format(round(sd_eks, 2), nsmall = 2)` $$

Et typisk avvik fra gjennomsnittet er altså omtrent `r format(round(sd_eks, 1), nsmall = 1)` likestillingspoeng for disse elevene.

Tallet inne i kvadratroten, `r format(round(sd_eks_var, 2), nsmall = 2)`, har et eget navn: det er **variansen**, altså det gjennomsnittlige kvadrerte avviket. Variansen er vanskelig å tolke direkte fordi enheten også er kvadrert; et "kvadrert likestillingspoeng" gir lite mening. Standardavviket er derimot uttrykt i samme enhet som dataene, nettopp fordi vi tok kvadratroten til slutt, og det er denne egenskapen som gjør det intuitivt. Måler vi for eksempel at elevene løper gjennomsnittlig 2300 meter på en 12-minutters løpstest med et standardavvik på 400 meter, så er et typisk avvik fra gjennomsnittet 400 meter. Du vil sjelden se variansen rapportert direkte, men den er grunnlaget for mange statistiske metoder og dukker opp igjen i senere kapitler.
