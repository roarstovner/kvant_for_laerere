...
tt(
  tibble(
    ...
    # `_Y_`/`_X_` gir kursiv i både HTML (via markdown) og typst. `math = TRUE`
    # ville fungert i typst, men får tinytable til å laste MathJax i HTML, og
    # den kolliderer med KaTeX-en boka bruker ("Math input error").
    `matematisk symbol` = c("_Y_", "_X_"),
  ...
  format_tt(i = 1, j = 2:3, markdown = TRUE)
```
