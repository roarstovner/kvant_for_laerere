...
# Y og X settes i kursiv med rå markup per format; tinytable slipper
# celleinnhold gjennom uendret. `format_tt(math = TRUE)` ville fått tinytable
# til å laste MathJax i HTML, og den kolliderer med KaTeX-en boka bruker
# ("Math input error"), mens `markdown = TRUE` krever pakken litedown.
symboler <- if (knitr::is_html_output()) c("<em>Y</em>", "<em>X</em>") else c("_Y_", "_X_")

tt(
  tibble(
    `variabelens rolle` = c("skal forklares", "skal forklare"),
    `matematisk symbol` = symboler,
